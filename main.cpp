...
// SIMD битовый вектор. Все операции идут по w активным словам, а не по всем 24.
struct alignas(32) Vec {
    ...
    void assign_xor(const Vec& a, const Vec& b, const Vec& c, const Vec& e, int w) {
        for (int i = 0; i < w; ++i) {
    ...
    void mask_to_n(int n, int w) {
        int full_words = n / 64;
        int rem = n % 64;
        if (rem > 0) d[full_words] &= (1ULL << rem) - 1;
        for (int i = full_words + (rem > 0 ? 1 : 0); i < w; ++i) d[i] = 0;
    }
...
int main() {
    ...
    Vec zero_vec;
    zero_vec.set_zero(MAX_WORDS);

    #pragma omp parallel for schedule(dynamic, 1) ordered
    for (int n = a; n <= b; ++n) {
        [[assume(n >= 1 && n <= MAX_N)]];

        const int words = (n + 64) / 64; // активных слов для n+1 бит
        ...
        for (int i = 0; i < n; ++i) {
            if (n == 1) {
                P_next[0].assign_xor(P_cur[0], zero_vec, zero_vec, P_prev[0], words);
                P_next[0].flip_bit(1);
            } else {
                // Крайний левый элемент (j = 0)
                P_next[0].assign_xor(P_cur[0], zero_vec, P_cur[1], P_prev[0], words);
                P_next[0].flip_bit(n);

                for (int j = 1; j < n - 1; ++j) {
                    P_next[j].assign_xor(P_cur[j], P_cur[j - 1], P_cur[j + 1], P_prev[j], words);
                    P_next[j].flip_bit(n);
                }

                // Крайний правый элемент (j = n - 1)
                P_next[n - 1].assign_xor(P_cur[n - 1], P_cur[n - 2], zero_vec, P_prev[n - 1], words);
        ...
        for (int i = 0; i < n - 1; ++i) {
            Vec left = board[i].shift_left(words);
            Vec right = board[i].shift_right(words);
            const Vec& prev = (i > 0) ? board[i - 1] : zero_vec;

            board[i + 1].assign_xor(board[i], left, right, prev, words);
            board[i + 1].flip_all(n);
            board[i + 1].mask_to_n(n, words);
